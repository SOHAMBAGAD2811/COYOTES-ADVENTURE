...
const STORY = [
  "THE BIOMASS SYNTHESIZER HUMS TO LIFE.",
  "WATER FLOWS. AIR CLEARS. SEEDS SPROUT.",
  "OASIS CITY WILL SURVIVE ANOTHER CYCLE.",
  "BUT THE AEGIS CORPORATION WILL RETURN.",
  "STAY VIGILANT, COYOTE. YOUR FIGHT HAS JUST BEGUN."
];
...
export default function StoryCardsEnd({ onComplete }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex w-full max-w-4xl flex-col gap-6 md:gap-8 items-center">
        {STORY.map((line, i) => (
          <motion.h1
            ...
            initial="hidden"
            animate="visible"
            className="glitch-text text-lg md:text-2xl font-bold tracking-[0.2em] md:tracking-[0.4em] text-[#00ff88]/90 shadow-glow"
            data-text={line}
          >
            {line}
          </motion.h1>
      ...
      <motion.button 
        ...
        className="absolute bottom-12 z-20 text-xs font-bold tracking-[0.3em] text-[#00ff88]/50 hover:text-[#00ff88] transition-colors animate-pulse"
        onClick={(e) => {
          e.stopPropagation();
          onComplete();
        }}
      >
        [ CLICK ANYWHERE TO COMPLETE MISSION ]
      </motion.button>
    </div>
  );
}
