...
export default function PixelButton({ 
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`pixel-button ${className} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
      onMouseEnter={disabled ? undefined : onMouseEnter}
      onMouseLeave={disabled ? undefined : onMouseLeave}
      onFocus={disabled || finalNoFocus ? undefined : onFocus}
      onBlur={disabled || finalNoFocus ? undefined : onBlur}
      tabIndex={disabled || finalNoFocus ? -1 : 0}
    >
      <canvas className="pixel-canvas" ref={canvasRef} />
      <span className="pixel-content">
        {children}
      </span>
    </button>
  );
